    ...
    <section class="content">
        <div class="row">
            ...
            <div class="box box-primary">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form role="form" action="{{ route('sale.update', ['sale' => $sale->id]) }}" method="POST">
                    <div class="box-body">
                        <div class="form-group">
                            <label for="input_name">Tên chương trình khuyến mãi</label>
                            <input type="text" class="form-control" id="input_name" name="name" placeholder="tên" value="{{ $sale->name }}">
                        </div>
                        <div class="form-group">
                            ...
                            <select name="sale_type_id" class="form-control">
                                ...
                                @foreach ($saleTypes as $each)
                                    <option value="{{ $each->id }}" {{ $sale->sale_type_id == $each->id ? 'selected' : '' }}>{{ $each->type_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="input_value">Giá trị</label>
                            <input type="number" name="value" id="input_value" class="form-control" value="{{ $sale->value }}">
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="input_date_start">Ngày bắt đầu</label>
                                <input id="input_date_start" class="form-control" type="date" name="date_start" value="{{ $sale->date_start }}">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="input_date_end">Ngày kết thúc</label>
                                <input id="input_date_end" class="form-control" type="date" name="date_end" value="{{ $sale->date_end }}">
                            </div>
                        </div>
                        <div class="form-group">
                                    ...
                                    <div class="modal-body">
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td scope="row">
                                                        <input type="checkbox" value="{{ $product->id }}" name="products[]" {{ $product->sale_id == $sale->id ? 'checked' : '' }}>
                                                    </td>
                                                    <td>{{ $product->name }}</td>
                                                <tr>
                                                @endforeach
